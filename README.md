...
```bash
npm install
npm start         # http://localhost:4100 (loopback only); demo data seeded on boot
```
...
| --- | --- | --- |
| `PORT` | `4100` | HTTP port |
| `HOST` | `127.0.0.1` (dev) / `0.0.0.0` (production) | Bind address; set `0.0.0.0` to expose the dev server on the LAN |
